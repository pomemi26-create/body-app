...
const lsGet = () => { try { const v=localStorage.getItem(SK); return v?JSON.parse(v):{meals:{},weights:{}}; } catch(e){ return {meals:{},weights:{}}; } };
...
function App(){
  ...
  const history=type=>{
    const map={};
    Object.values(data.meals).flat().filter(m=>m.type===type&&!m.dOut)
      .forEach(m=>{if(!map[m.name])map[m.name]={...m,n:0};map[m.name].n++;});
    return Object.values(map).sort((a,b)=>b.n-a.n).slice(0,30);
  };
  const filtered=history(mealType).filter(h=>!hQ||h.name.includes(hQ));
  ...
  const addMeal=()=>{
    let nm;
    if(dOut){
      nm={...data,meals:{...data.meals,[today]:[...meals,{type:mealType,name:"外食",kcal:0,p:0,f:0,c:0,dOut:true,id:Date.now()}]}};
      setDOut(false);
    } else {
      if(!fName||!kcal) return;
      nm={...data,meals:{...data.meals,[today]:[...meals,{type:mealType,name:fName,kcal:+kcal,p:+prot||0,f:+fat||0,c:+carb||0,id:Date.now()}]}};
  ...
  const buildSummary=()=>{
    const lines=[
      ...
      `目標: ${WD_KCAL}kcal P${WD_P}g F${WD_F}g C${WD_C}g`,
    ];
    return lines.join("\n");
  };
  ...
  return (
    <div style={{fontFamily:"'Hiragino Sans','Yu Gothic',sans-serif",background:"linear-gradient(135deg,#FFF0F5,#F0F8FF)",minHeight:"100vh",maxWidth:480,margin:"0 auto",paddingBottom:80}}>
      ...
      <div style={{padding:"16px 16px 0"}}>
          ...
          {card(<>
            ...
            {dOut?(
              ...
              <button onClick={()=>setShowH(!showH)} style={{width:"100%",padding:10,borderRadius:12,border:"2px dashed #FF9F43",background:"white",color:"#FF9F43",fontWeight:"bold",cursor:"pointer",marginBottom:10}}>
                {showH?"▲ 履歴を閉じる":"🕐 "+mealType+"の履歴（"+history(mealType).length+"件）"}
              </button>
              {showH&&(
                <div style={{marginBottom:10,background:"#FFF8F0",borderRadius:14,padding:12}}>
                  <input value={hQ} onChange={e=>setHQ(e.target.value)} placeholder="食品名で絞り込む..." style={{...IS,marginBottom:8}}/>
                  {filtered.length===0?(
                    <div style={{textAlign:"center",color:"#aaa",fontSize:13,padding:"8px 0"}}>{history(mealType).length===0?"まだ履歴がありません":"該当なし"}</div>
                  ):(
                      ...
                      {filtered.map((item,i)=>(
                        <div key={i} onClick={()=>applyH(item)} style={{padding:"9px 12px",borderRadius:10,marginBottom:6,background:"white",cursor:"pointer",fontSize:13,display:"flex",justifyContent:"space-between",alignItems:"center",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
                          ...
                          <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:2}}>
                            ...
                            <span style={{fontSize:10,color:"#ccc"}}>{item.n}回</span>
                          </div>
                        </div>
                      ))}
                    </div>
        ...
        {tab===4&&<>
          {card(<>
            ...
            <div style={{fontSize:13,color:"#888",marginBottom:16,lineHeight:1.8}}>
              今日の記録をコピーして、ClaudeにAIアドバイスをもらいましょう🌸<br/>
              下のボタンで記録をコピー→Claudeアプリに貼り付けて送信してください。
            </div>
            ...
            <button onClick={()=>{navigator.clipboard&&navigator.clipboard.writeText(buildSummary()).then(()=>alert("コピーしました！Claudeアプリに貼り付けてください🌸"));}} style={{width:"100%",padding:13,borderRadius:12,border:"none",background:"linear-gradient(135deg,#A29BFE,#FF6B9D)",color:"white",fontWeight:"bold",fontSize:15,cursor:"pointer",marginBottom:10}}>
              📋 今日の記録をコピー
            </button>
            <a href="https://claude.ai" target="_blank" style={{display:"block",width:"100%",padding:13,borderRadius:12,border:"none",background:"linear-gradient(135deg,#FF6B9D,#FF9F43)",color:"white",fontWeight:"bold",fontSize:15,cursor:"pointer",textAlign:"center",textDecoration:"none"}}>
              🤖 Claudeを開く
            </a>
          </>)}
          {card(<>
            <div style={{fontWeight:"bold",fontSize:14,color:"#555",marginBottom:10}}>📌 計算済みの設定</div>
            <div style={{fontSize:13,color:"#666",lineHeight:2}}>
              <div>🧮 BMR: {Math.round(BMR)} kcal</div>
              <div>⚡ TDEE: {TDEE} kcal</div>
              <div>📅 平日: {WD_KCAL}kcal P{WD_P}g F{WD_F}g C{WD_C}g</div>
              <div>🎉 リフィード: {RF_KCAL}kcal P{RF_P}g F{RF_F}g C{RF_C}g</div>
              <div>📉 月間予測: 約0.5〜0.7kg減</div>
            </div>
            <div style={{marginTop:10,padding:10,background:"#FFF0F5",borderRadius:12,fontSize:12,color:"#FF6B9D",lineHeight:1.6}}>
              💡 脂質で太りやすい体質のため平日は{WD_F}g上限に。炭水化物{WD_C}g以上で代謝を守りましょう🌸
            </div>
          </>)}
        </>}
      ...
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,background:"white",borderTop:"1px solid #f0f0f0",display:"flex",boxShadow:"0 -4px 20px rgba(0,0,0,0.08)"}}>
        {TABS.map((t,i)=>(
          <button key={i} onClick={()=>setTab(i)} style={{flex:1,padding:"12px 4px 10px",border:"none",background:"transparent",cursor:"pointer",fontSize:10,fontWeight:"bold",color:tab===i?"#FF6B9D":"#aaa",borderTop:tab===i?"3px solid #FF6B9D":"3px solid transparent"}}>{t}</button>
        ))}
      </div>
    </div>
  );
